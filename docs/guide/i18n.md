...
// Apply locale-specific fonts
themeService.applyFontStylesheet(newLocale);
...
To persist the user's language preference across sessions, use `UserPreferencesService` (see [User Preferences](preferences.md)):

```java
preferencesService.setLocale(newLocale);
```

The saved locale is restored automatically on the next application launch.
